import { subscriptionTiers, TierNames } from "@/data/subscription-tier";
...
export const ProducTable = pgTable(
  "products",
  {
...
export const productsRelations = relations(ProducTable, ({ one, many }) => {
  return {
...
export const ProductCustomizationTable = pgTable("product_customizations", {
  ...
  productId: uuid("product_id")
    .notNull()
    .references(() => ProducTable.id, { onDelete: "cascade" })
    .unique(),
...
export const productsCustomizationRelations = relations(
  ProductCustomizationTable,
  ({ one }) => {
    return {
      product: one(ProducTable, {
        fields: [ProductCustomizationTable.productId],
        references: [ProducTable.id],
      }),
    };
  }
);
...
export const productsViewRelations = relations(ProductViewTable, ({ one }) => {
  return {
    product: one(ProducTable, {
...
export const countriesRelations = relations(CountryTable, ({ one, many }) => {
  return {
...
export const countryGroupsRelations = relations(
  CountryGroupTable,
  ({ many }) => {
    return {
